...
def right_multiplication(matrix, posx, posy):
    if posy > 16:
        return 0
    product = matrix[posx][posy] * matrix[posx][posy + 1] * \
        matrix[posx][posy + 2] * matrix[posx][posy + 3]
    return product


def down_multiplication(matrix, posx, posy):
    if posx > 16:
        return 0
    sum = matrix[posx][posy] * matrix[posx + 1][posy] * \
        matrix[posx + 2][posy] * matrix[posx + 3][posy]
    return sum


def right_diagonal_multiplication(matrix, posx, posy):
    if posx > 16 or posy > 16:
        return 0
    sum = matrix[posx][posy] * matrix[posx + 1][posy + 1] * \
        matrix[posx + 2][posy + 2] * matrix[posx + 3][posy + 3]
    return sum


def left_diagonal_multiplication(matrix, posx, posy):
    if posx < 3 or posy > 16:
        return 0
    sum = matrix[posx][posy] * matrix[posx - 1][posy + 1] * \
        matrix[posx - 2][posy + 2] * matrix[posx - 3][posy + 3]
    return sum
...
def find_multiplication(matrix):
    max_value = 0
    for i in range(len(matrix)):
        for j in range(len(matrix[i])):
            right_product = (right_multiplication(matrix, i, j))
            down_produdct = down_multiplication(matrix, i, j)
            diagonal_product = right_diagonal_multiplication(matrix, i, j)
            left_diagonal_product = left_diagonal_multiplication(matrix, i, j)

            max_value = max(max_value, right_product, down_produdct,
                            diagonal_product, left_diagonal_product)
    return max_value


print(find_multiplication(matrix))
